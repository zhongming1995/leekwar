window.LanguageTranslation= window.LanguageTranslation || {};
(function (exports,cc) {
    'use strict';
    class LanguageMgr {
        ...
        static async init(type) {
            ...
            LanguageMgr.languageJSon = await this.load_json('language.json');
        }

        static load_json(json_name) {
            let json_url = 'config/' + json_name;
            return new Promise((resolve, reject) => {
                cc.loader.loadRes(json_url, (err, data) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(data.json);
    ...
    LanguageMgr.languageInit = false;
    ...
    Object.defineProperty(cc.Label.prototype, 'string', {
        ...
        get() {
            LanguageTranslation && (this._string = LanguageTranslation.LanguageMgr.getStr(this._string));
            return this._string;
        }
    });
}(window.LanguageTranslation = window.LanguageTranslation || {},cc))